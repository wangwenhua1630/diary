        ...
        >
          <div class="c_item" @click="goDetail(item.id)" v-for="(item,index) in lists">
            ...
            <div class="c_item_imgList" v-if="item.imgs.length!=4&&item.imgs.length!=1">
              <div class="c_item_img" v-for="(items,indexs) in item.imgs" @click.stop="lookImgss(item.imgs,indexs)">
                <img :src="items.img" alt="">
              </div>
            </div>
            <!----等于四张图-->
            <div class="c_item_imgList1" v-if="item.imgs.length==4">
              <div class="c_item_img1" v-for="(items,indexs) in item.imgs" @click.stop="lookImgss(item.imgs,indexs)">
                <img :src="items.img" alt="">
              </div>
            </div>
            <!--一张图时-->
            <div class="c_item_imgList" v-if="item.imgs.length==1" @click.stop="lookImgss(item.imgs,0)">
  ...
  Vue.use(ImagePreview);
    export default {
      ...
      created(){
        ...
        //   message:'加载中'
        // })
        getDiaryList(1,'',this.ye).then(res=>{
          // this.$toast.clear();
          console.log(res);
          if(res.data.flag){
